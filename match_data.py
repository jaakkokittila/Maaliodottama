...
def get_matches(match_file_path):
    try:
        return pd.read_csv(match_file_path, encoding='cp1252')
    except:
        return None

def match_exists(game_id, saved_matches):
    if saved_matches is None:
        return []
    else:
        return saved_matches[saved_matches['Match_id'] == int(game_id)]
...
def write_matches(game_ids, season):
    ...
    saved_matches = get_matches(match_file_path)
    ...
    with open(match_file_path, mode=write_mode, encoding='cp1252') as match_file:
        field_names = ['Match_id', 'Home_id', 'Home_name', 'Away_id', 'Away_name',
        ...
        for game_id in game_ids:
            ...
            matchexists = match_exists(match_id, saved_matches)

            if len(matchexists) == 0:
                ...
                match_writer.writerow({'Match_id': match_id, 'Home_id': home_id, 'Home_name': home_name,
